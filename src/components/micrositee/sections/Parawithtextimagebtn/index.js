...
export default function Parawithtextimagebtn({ details, AR}){
 const aspectRatio = AR || 32 / 49;
  ...
  const descriptiondata=media.find(item => item.a_title === "Popular Categories")?.a_description
  const bgData =  sortedMedia.filter(item => item.a_media_type === 'BackgroundImage');
