...
import { Modal, View, Text, TextInput, TouchableOpacity, Image, ScrollView, Platform, KeyboardAvoidingView, StatusBar } from 'react-native';
...
const VerifyScreen = ({ state, update, onNext, onBack }) => {
  ...
  useEffect(() => {
    timerRef.current = setInterval(() => {
      update('timer', prev => prev <= 1 ? (clearInterval(timerRef.current), 0) : prev - 1);
    }, 1000);
    resendRef.current = setInterval(() => {
      update('resendTimer', prev => prev <= 1 ? (clearInterval(resendRef.current), 0) : prev - 1);
    }, 1000);
    return () => {
  ...
  const handleVerify = () => {
    if (state.timer === 0)      { update('error', T.errors.expired);  return; }
    if (state.otp.length < 4)   { update('error', T.errors.invalid);  return; }
    if (state.otp !== '123456') {
  ...
  return (
    <View style={styles.screen}>
      ...
      <Text style={styles.otpInfo}>{T.otpInfoPrefix}{state.mobile}</Text>
      <View style={styles.inputWrapper}>
        <TextInput
          ...
          maxLength={6}
          value={state.otp}
          onChangeText={v => { update('otp', v); update('error', ''); }}
          editable={state.attempts > 0 && state.timer > 0}
          placeholderTextColor="#BBBBBB"
        />
        <Text style={styles.remainingTime}>
          {T.remainingLabel}{formatTime(state.timer)}
        </Text>
      </View>
      <View style={styles.resendRow}>
        {state.resendTimer > 0 ? (
          <Text style={styles.resendWait}>
            {T.resendWaitPrefix}{formatTime(state.resendTimer)}
          </Text>
        ) : (
      ...
      {state.attempts < 3 && (
        <Text style={styles.attemptsText}>{T.attemptsLabel(state.attempts)}</Text>
      )}
      {state.error ? <Text style={styles.errorText}>{state.error}</Text> : null}
      <PrimaryButton
        label={T.btnLabel}
        onPress={handleVerify}
        disabled={state.otp.length < 4 || state.attempts === 0 || state.timer === 0}
      />
    </View>
  );
};
const SignupScreen = ({ state, update, onSubmit, onBack }) => {
  ...
  const handleSubmit = () => {
    if (!state.firstName)                          { update('error', T.errors.firstName); return; }
    if (!state.email || !state.email.includes('@')) { update('error', T.errors.email);     return; }
    if (!state.agreed)                             { update('error', T.errors.terms);      return; }
    onSubmit();
  };
  return (
    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.inputReadOnly}>
        ...
        <Text style={styles.inputValue}>{T.mobilePrefix}{state.mobile}</Text>
      </View>
      ...
      <View style={styles.genderRow}>
        {T.genderOptions.map(g => (
          ...
          >
            <View style={state.gender === g ? styles.radioOuterSelected : styles.radioOuter}>
